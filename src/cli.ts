...
function splitArgv(argv: readonly string[]): { head: readonly string[]; command: readonly string[] } {
  const dd = argv.indexOf('--');
  if (dd >= 0) return { head: argv.slice(0, dd), command: argv.slice(dd + 1) };
  return { head: [...argv], command: [] };
}

function parsePortValue(next: string | undefined): number | ParsedCommand {
...
function parseHostFlags(flags: readonly string[]): { port?: number; name?: string } | ParsedCommand {
...
function parseHostArgs(flags: readonly string[], command: readonly string[]): ParsedCommand {
  const parsed = parseHostFlags(flags);
  if ('cmd' in parsed) return parsed;
  if (command.length === 0) {
    return err('host needs a command after "--", e.g. `vibelive host -- claude`');
  }
  return { cmd: 'host', command, port: parsed.port, name: parsed.name };
}

function parseJoinArgs(flags: readonly string[]): ParsedCommand {
...
export function parseArgs(argv: readonly string[]): ParsedCommand {
  const { head, command } = splitArgv(argv);
  ...
  if (sub === 'host') return parseHostArgs(head.slice(1), command);
  if (sub === 'join') return parseJoinArgs(head.slice(1));
...
function printHostBanner(command: readonly string[], relay: { url: string; port: number }): void {
  process.stderr.write(`vibelive host ready — sharing ${command.join(' ')}\n`);
...
async function runHost(p: { readonly command: readonly string[]; readonly port?: number; readonly name?: string }): Promise<number> {
  ...
  printHostBanner(p.command, relay);

  const rl = createInterface({ input: process.stdin, terminal: false });
  const shutdown = makeHostShutdown(rl, host, relay);
  rl.on('line', (line) => handleHostLine(line, host, relay, shutdown));
...
function wireJoinClient(client: SessionClient): void {
...
function handleJoinLine(line: string, client: SessionClient, rl: ReadlineInterface): void {
  if (line === '/quit') {
    client.close();
    rl.close();
    return;
...
async function runJoin(p: { readonly url: string; readonly name?: string }): Promise<number> {
  ...
  const rl = createInterface({ input: process.stdin, terminal: false });
  rl.on('line', (line) => handleJoinLine(line, client, rl));
  rl.on('SIGINT', () => {
    client.close();
    rl.close();
  });
